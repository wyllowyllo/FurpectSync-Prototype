...
### 미완료 (Unity Editor 수동 작업 필요)
- [ ] PUN2 임포트 + AppId 설정 (Asset Store에서 다운로드 필요)
- [ ] Cinemachine 설치 (Package Manager → `com.unity.cinemachine`)
- [ ] TextMeshPro 설치 (Package Manager → TMP Essential Resources 임포트)
- [ ] TestScene 생성 (Plane, Light, Camera, NetworkManager 오브젝트, SpawnPoints)
- [ ] TestScene에 CinemachineCamera 추가 + Body 설정
- [ ] ConvergenceCharacter.prefab 생성 (Resources/ — Capsule + PhotonView + ConvergenceController + CharacterController)
- [ ] PhotonView에 ConvergenceController를 Observed Component로 등록
- [ ] 팀 머티리얼 생성 (TeamA=Blue, TeamB=Red) — 또는 런타임 색상 적용으로 대체 가능
- [ ] DebugUI Canvas 구성 (Canvas + TMP 텍스트 5개 + DebugUI 컴포넌트 연결)
- [ ] 2클라이언트 테스트 (ParrelSync 또는 빌드+에디터)
